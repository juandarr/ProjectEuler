...
def contains_origin_checker(filename):
    ...
    for t in triangles:
        ...
        # Checks whether all points are at the same side of one axis, this means the origin is not included
        if all([point[0]>0 for point in t]) or all([point[0]<0 for point in t]) or all([point[1]>0 for point in t]) or all([point[1]<0 for point in t]):
            continue
        else:
