"""
Finds the number of characters saved by writing each number in a minimal form
Author: Juan Rios
"""
import math
...
def read_number(value, roman_dict):
    ...
    while index < len(value):
        if index<len(value)-1:
            if value[index:index+2] in ['IV','IX','XL','XC','CD','CM']:
                number += roman_dict[value[index:index+2]]
                index += 2
                continue
        number += roman_dict[value[index]]
        index += 1
    return number
...
def characters_saved(roman_dict):
    ...
    for roman in romans:
        ...
        roman_minimal = minimal_roman(number)
        sub += len(roman)-len(roman_minimal)
    return sub 

if __name__ == "__main__":
    values = {'I':1,'V':5,'X':10,'L':50,'C':100,'D':500,'M':1000,'IV':4,'IX':9,'XL':40,'XC':90,'CD':400,'CM':900}
    print('The number of characters saved when changing to minimal form is {0}'.format(characters_saved(values))) 